...
trait MysqlTable
{
    ...
    public function saveOperation($request_data)
    {
        if (isset($request_data[$this->primaryKey])) $model = $this->find($request_data[$this->primaryKey]);
        if (empty($model)) $model = $this;
        $columes = Schema::getColumnListing($model->getTable());
        foreach ($request_data as $k => $v) {
            if (
                (
                    ...
                    &&
                    array_key_exists($k, $request_data)
                )
                && $k != $this->primaryKey
                && in_array($k, $columes)
            ) {
                $model->$k = is_null($v) ? '' : htmlspecialchars($v, ENT_QUOTES);
            }
        }
        $model->save();
        return $model;
