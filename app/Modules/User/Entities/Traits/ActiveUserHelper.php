<?php
namespace App\Modules\User\Entities\Traits;
...
// 用于存放临时用户数据
    ...
    private function calculateActiveUsers()
    {
        ...
        foreach ($users as $user_id => $user) {
            // 找寻下是否可以找到用户
            $user = $this->with('userInfo')->find($user_id);
    ...
    private function calculateTopicScore()
    {
        ...
        $topic_users = Dynamic::select(DB::raw('user_id, count(*) as topic_count'))
            ->where(Dynamic::CREATED_AT, '>=', strtotime(Carbon::now()->subDays($this->pass_days)))
            ->groupBy('user_id')
            ->get();
        // 根据话题数量计算得分
    ...
    private function calculateReplyScore()
    {
        ...
        $reply_users = DynamicComment::select(DB::raw('user_id, count(*) as reply_count'))
            ->where(DynamicComment::CREATED_AT, '>=', strtotime(Carbon::now()->subDays($this->pass_days)))
            ->groupBy('user_id')
            ->get();
        // 根据回复数量计算得分
